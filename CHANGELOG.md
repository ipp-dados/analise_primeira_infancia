...
Histórico completo de mudanças em `analise.py` e no projeto. O `README.md` mantém só as
entradas mais recentes; esta é a lista completa, entrada nunca reescrita — uma mudança de
convenção vira uma entrada nova, não uma edição da antiga (ver `specs/constitution.md`).

*Nota (reorganização de 2026-09-22): pastas `SPEC-*` citadas abaixo foram movidas para
`specs/<nome>` (ex.: `SPEC-mortalidade-AP/` → `specs/mortalidade-ap/`); `relatorio/index.html`
foi renomeado para `relatorio/relatorio.html` (2026-09-22, mesma sessão). As entradas abaixo
preservam os nomes originais como registro histórico — ver `specs/constitution.md` e
`specs/roadmap.md` para o estado atual.*
