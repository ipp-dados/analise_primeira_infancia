...
- `relatorio/` — `index.html`, a self-contained interactive HTML report (SVG charts + choropleths, light/dark theme), generated by `.claude/skills/export_pdf_report/scripts/build_html_report.py`. **Distinct pipeline from `analise_primeira_infancia.pdf`** (same skill folder, different script, different visual identity — see the `export_pdf_report` skill's note before touching either). `index.html` is explicitly exempted from `.gitignore` (`!/relatorio/index.html`) and committed, same as the PDF — GitHub Pages deploys it as-is.
