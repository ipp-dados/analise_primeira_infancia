# Especificação — Inclusão dos dados do eixo Proteção (`specs/inclusao_dados_protecao`)
...
> **Restrição herdada de `specs/mortalidade-ap` e `specs/ajuste_eixos`: só se acrescenta.**
...
(markdown-only) — ordem técnica de build, coerente com `specs/ajuste_eixos`; a apresentação por eixo
...
(`gera_docx_curadoria.py`; textos por visualização, `specs/ajuste_eixos` §7). 7. Skills/docs.
