...
`specs/maps-and-ibge`, `specs/visual-identity` e `specs/relatorio-interativo`),
...
Apesar do branch estar muito atrasado, as mudanças de `specs/maps-and-ibge`
+ `specs/visual-identity` + `specs/relatorio-interativo` não tocam as mesmas
...
1. Fazer o merge de verdade (`git merge origin/waleska-analise-primeira-infancia`,
   ...
   `specs/merge-waleska-changes` (criado a partir de `planning`).
